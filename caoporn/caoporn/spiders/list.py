...
class ListSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        start_cursor = db[collection_name].find().limit(1).sort([('$natural', -1)])
        if start_cursor.count() == 0:
            start_url = self.start_url
        else:
            start_url = start_cursor[0]['page_url']
            self.log("start with last checkpoint %s" %(start_url))
        return [scrapy.Request(url=start_url,callback=self.parse_video)]
    ...
    def parse_video(self, response):
        for vid in response.xpath('//div[@class="video_box"]'):
            ...
            item['hash'] = vid.xpath('a/@href').re('/video/([0-9a-f]+)/')[0]
